...
const non_replacement_weighted_sampler_1 = require("./non-replacement-weighted-sampler");
function testReasonableSamplesOrder() {
    ...
    const sampler = new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(items, respectiveWeights);
    const maxReasonableNonDescendingValues = 2 * Math.ceil(Math.log2(items.length));
    const sampledItemsInOrder = [];
    // Act.
...
function testLogarithmicComplexity(samplesCount, sampleWeight, maxReasonableRestructureAttempts) {
    // Arrange.
    const respectiveWeights = new Array(samplesCount)
        .fill(0)
        .map(_ => sampleWeight());
    ...
    const previouslySampledItems = new Set();
    do {
...
describe('NonReplacementWeightedSampler tests', () => {
    describe('Happy path tests', () => {
        test('validating that remaining items are sampled proportionally to their weights, ' +
        ...
        test('validating that the number of restructure operations is O(logn) for random weights, ' +
            ...
            const maxWeight = 114;
            const sampleWeight = () => maxWeight * Math.random();
    ...
    describe('Negative path tests', () => {
        test('constructor should throw an error when given 0 items', () => {
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler([], [])).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler([], [4.77])).toThrow();
        });
        test('constructor should throw an error when the number of items does not match the number of weights', () => {
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2"], [1, 2, 3])).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2"], [51])).toThrow();
        });
        test('constructor should throw an error when the failedSampleAttemptsBeforeRestructure ' +
            'argument is a non-natural number', () => {
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2"], [1, 2], 0)).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2"], [3, 4], -1)).toThrow();
        });
        test('constructor should throw an error when a non-positive weight is provided', () => {
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2"], [1, -2])).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2"], [0, 5.6])).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2", "item3"], [0.3, 4, 0])).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2", "item3"], [6, 4, -0.7])).toThrow();
        });
        test('constructor should throw an error when an undefined item value is provided', () => {
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", undefined], [1, 2])).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler([undefined, "item2"], [4, 6])).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", "item2", undefined], [1, 4, 3])).toThrow();
            expect(() => new non_replacement_weighted_sampler_1.NonReplacementWeightedSampler(["item1", undefined, "item3"], [6, 4, 1])).toThrow();
        });
    });
});
//# sourceMappingURL=non-replacement-weighted-sampler.test.js.map